...
class TicTacToe():
    ...
    def is_winning(self, curr_state):
        ...
        #check the sum of diagonal from top to bottom
        if (curr_state[0] + curr_state[4] + curr_state[8])==15:
            return True
            #check the sum of diagonal from bottom to top
    ...
    def step(self, curr_state, curr_action):
        ...
        agent_action, env_actions = self.action_space(next_state)
        random_env_action = random.choice(env_actions)
        next_state = self.state_transition(next_state , random_env_action)
        terminal , result = self.is_terminal(next_state)
